...
export class WowClicker {
  ...
  action() {
    ...
    items.tree.status = this.setTreeStatus(items.tree);
    ...
    formElement.addEventListener("submit", (event) => {
      ...
      if (event.submitter.id === "add-total") {
        total += 1 + items.o.own;
      } else if (event.submitter.id === "add-autoclicker" && items.autoclicker.cost <= total) {
      ...
      } else if (event.submitter.id === "add-tree" && items.tree.cost <= total) {
  ...
  refreshView(total, wps, items) {
    ...
    const addTree = document.querySelector("#add-tree");
    const plant = document.querySelector("#plant");
    const ownNekochanElement = document.querySelector("#own-nekochan");
    const costNekochanElement = document.querySelector("#cost-nekochan");
    ...
    this.updateHTML("owned: " + items.autoclicker.own.toLocaleString(), ownAutoclickerElement);
    this.updateHTML("cost: " + items.autoclicker.cost.toLocaleString() + " wow!", costAutoclickerElement);
    this.updateHTML("owned: " + items.o.own.toLocaleString(), ownOElement);
    this.updateHTML("cost: " + items.o.cost.toLocaleString() + " wow!", costOElement);
    this.updateHTML("cost: " + items.tree.cost.toLocaleString(), costTree);
    this.updateHTML("ascend: " + items.tree.ascend.toLocaleString(), ascendTree);
    const harvest = items.tree.harvest + items.tree.ascend;
    this.updateHTML("mature: " + items.tree.maturity.toLocaleString() + "/" + harvest.toLocaleString(), matureTree);
    this.updateHTML(items.tree.view.toLocaleString(), plant);
    this.updateHTML(items.tree.status, addTree);
  }
